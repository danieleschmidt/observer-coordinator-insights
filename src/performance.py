...
class CachedDataProcessor:
    ...
    def cached_clustering(self, features: pd.DataFrame, n_clusters: int,
        ...
        if result is not None:
            logger.debug("Clustering result found in memory cache")
            return result

        # Try disk cache
        result = self._load_from_disk_cache(cache_key)
        if result is not None:
            logger.debug("Clustering result found in disk cache")
        ...
        logger.debug("Clustering result computed and cached")
        return result
